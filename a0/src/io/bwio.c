...
int bwsetfifo(int channel, int state) {
    ...
    switch (channel) {
    case COM1:
        line = (int*) (UART_LCRH_OFFSET + UART1_BASE);
        break;
    case COM2:
        line = (int*) (UART_LCRH_OFFSET + UART2_BASE);
        break;
    default:
        return E_FAIL;
        break;
...
int bwsetspeed(int channel, int speed) {
    ...
    switch (channel) {
    case COM1:
        high = (int*) (UART_LCRM_OFFSET + UART1_BASE);
        low = (int*) (UART_LCRL_OFFSET + UART1_BASE);
        break;
    case COM2:
        high = (int*) (UART_LCRM_OFFSET + UART2_BASE);
        low = (int*) (UART_LCRL_OFFSET + UART2_BASE);
        break;
    default:
        return E_FAIL;
        break;
...
int bwputc(int channel, char c) {
    ...
    switch (channel) {
    case COM1:
        flags = (int*) (UART_FLAG_OFFSET + UART1_BASE);
        data = (int*) (UART_DATA_OFFSET + UART1_BASE);
        break;
    case COM2:
        flags = (int*) (UART_FLAG_OFFSET + UART2_BASE);
        data = (int*) (UART_DATA_OFFSET + UART2_BASE);
        break;
    default:
        return E_FAIL;
        break;
    ...
    while ((*flags & TXFF_MASK)) {
        // busy wait while transmit buffer full
    }
...
int bwgetc(int channel) {
    ...
    while (!(*flags & RXFF_MASK)) {
        // busy wait while receive buffer is not full
    }

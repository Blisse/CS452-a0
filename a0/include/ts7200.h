...
static inline int* ADDRESS_OF_UART(int channel, int offset) {
    if (channel == COM1) {
        return (int*) (offset + UART1_BASE);
    } else {
        return (int*) (offset + UART2_BASE);
    }
}

static inline int* ADDRESS_OF_TIMER(int timer, int offset) {
    if (timer == TIMER1) {
        return (int*) (offset + TIMER1_BASE);
    } else if (timer == TIMER2) {
        return (int*) (offset + TIMER2_BASE);
    } else {
        return (int*) (offset + TIMER3_BASE);
    }
}
